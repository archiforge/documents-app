...
/// Adds `pageCount` to documents so PDF rows can badge their page count
...
/// Current schema. Adds `createdAt` — the file's actual creation date — so
/// lists can order and label documents by when they came into existence,
/// not when the app imported them.
enum SchemaV3: VersionedSchema {
    ...
    final class DocumentRecord {
        ...
        /// badge without re-parsing the file. nil = not a PDF or not yet known.
        var pageCount: Int? = nil
        /// The file's actual creation date (`creationDateKey`), independent
        /// of when the app imported or adopted it. nil = not yet known;
        /// `creationDate` falls back to `importedAt`. Startup recovery
        /// backfills older records.
        ...
        /// The date the document came into existence: the file's real
        /// creation date when known, else when the app imported it.
...
typealias DocumentRecord = SchemaV3.DocumentRecord

/// The current folder-grant model (see `DocumentRecord` alias).
typealias FolderGrant = SchemaV3.FolderGrant
