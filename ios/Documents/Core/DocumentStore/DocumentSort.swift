...
/// in which direction. Date orders by creation date — the file's actual
/// creation time (`createdAt`), falling back to import time for records it
/// is not known for yet. Pure value logic mirroring `FormatFilter` so
/// ordering is unit-testable without SwiftData queries.
struct DocumentSort: Equatable, Sendable {
    ...
    private func less(_ lhs: DocumentRecord, _ rhs: DocumentRecord) -> Bool {
        ...
        switch field {
        case .date:
            primary = lhs.creationDate.compare(rhs.creationDate)
        case .name:
